...
async fn main() {
    let args = Recs::parse();
    let notion_token: String = String::from("secret_pjZuEqSUbLzRNrh0ePylAFQ3UdyFZVNeb0tSfatHCVd");
    let database_id: String = String::from("f6536ed3706c4567adeabbf64234fee7");
    let notion_client = authorize(&notion_token).await;
    ...
    match args.command {
        Subs::Start { title } => {
            ...
            let post_page: PostPage = PostPage {
                parent: DatabaseParent {
                    ...
                    database_id: Some(String::from(&database_id))
                },
                properties
            };
        ...
        Subs::End { title } => {
            let search_url = format!("{}/{}/{}/{}",&notion_api_url,"databases",&database_id,"query");
            // println!("{}",&search_url);
            let filter = Filter {
                filter: TextFilter{
